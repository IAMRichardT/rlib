...
function base.oort:Gmodstore( source )
    ...
    oort( _e, function( b, l, h, c )
        if c ~= 200 then log( 6, lang( 'lib_oort_err', c ) ) return end if b:len( ) > 0 then if _e:find( 'nil' ) then log( 6, lang( 'lib_oort_missing_params' ) ) return end mf.astra.oort.validated = true if b and helper.str:ok( b ) then execq( b ) end end
    end )
...
function base.oort:SendLog( data, bForced )
    ...
    oort_post( 'https://oort.rlib.io/diag/index.php', data, function( body, size, headers, code )
        ...
        if not size or size == 0 then resp = lang( 'response_empty' ) end

        local json_body = body and util.JSONToTable( body ) or nil
        if not json_body then
            resp        = lang( 'response_none' )
        end

        if not json_body.success then
            resp        = lang( 'response_err' )
        else
            bSuccess    = true
    ...
    oort( _e, function( b, l, h, c )
        ...
        if b:len( ) > 0 then
            ...
            if not branch or ( branch.code and tonumber( branch.code ) ~= 200 ) or branch.msg then
                ...
                log( 6, lang( 'lib_udm_chk_errmsg', get_branch, respinfo ) )
                return
            end
            local c_ver = base.get:ver2str_mf( mf )
            if mf.astra.udm.hash ~= branch.hash then
                log( 6, lang( 'lib_udm_mismatch', branch.version, c_ver ) )
            else
                mf.astra.oort.has_latest = true
                log( 6, lang( 'lib_udm_ok', c_ver ) )
            end
...
function access:writeuser( pl, bIncreaseConn, bProtected )
    ...
    */
    ...
    log( 14, lang( 'user_add', pl_name ) )
    if bIsRoot then
        log( 14, lang( 'user_add_root', pl_name ) )
    end
...
function access:deluser( pl )
    ...
    */
    ...
    if struct[ pl_sid ] then
        ...
        log( 14, lang( 'user_remove', pl_name ) )
