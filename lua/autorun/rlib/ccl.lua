...
function konsole:send( cat, ... )
    cat = isnumber( cat ) and cat or 0

    if not ui:ok( self.pnl ) and access:bIsDev( LocalPlayer( ) ) then
        self.pnl = vgui.Create( 'rlib.lo.konsole' )
        self.pnl:ActionHide( )
    end

    if not ui:ok( self.pnl ) then return end

    self.pnl:AddEntry( cat, { ... } )
end
...
function konsole:send_varg( cat, msg, ... )
    cat = isnumber( cat ) and cat or 0
    ...
    self.pnl:AddEntry( cat, msg, true )
end
